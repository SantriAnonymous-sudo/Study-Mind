...
export default function AuthModal({ onClose, onSuccess, initialMode = 'login' }: AuthModalProps) {
  ...
  const handleGoogleSocialLogin = async () => {
    ...
    try {
      ...
      if (!firebaseUser.email) {
        throw new Error('Gagal mendapatkan informasi email dari Autentikasi Google.');
      }
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Autentikasi gagal disinkronkan ke peladen.');
      }

      setSuccessMsg(`Autentikasi Google berhasil! Masuk sebagai ${firebaseUser.email}...`);
      safeStorage.setItem('studymind_userId', data.user.uid);
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        setError('Pendaftaran/masuk dibatalkan karena jendela popup ditutup.');
      } else {
  ...
  const handleGithubSocialLogin = async () => {
    setLoading(true);
    setError(null);
    setSuccessMsg(null);
    try {
      ...
      const primaryEmail = firebaseUser.email || firebaseUser.providerData[0]?.email || `${firebaseUser.uid}@github.invalid`;

      const res = await fetch('/api/auth/github-sso', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          fullName: firebaseUser.displayName || (firebaseUser as any).reloadUserInfo?.screenName || 'GitHub Scholar',
          email: primaryEmail
        })
      });
      const data = await res.json();
      ...
      setSuccessMsg(`Autentikasi GitHub berhasil! Masuk sebagai ${primaryEmail}...`);
      safeStorage.setItem('studymind_userId', data.user.uid);
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setError(err.message || 'Terjadi kesalahan saat masuk menggunakan GitHub.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-slate-900 bg-slate-950 p-6 shadow-2xl">
        ...
        {authType === 'instant' && (mode === 'login' || mode === 'register') ? (
          ...
          <div className="space-y-4">
            {/* Real Firebase Social Logins */}
            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={handleGoogleSocialLogin}
                disabled={loading}
                className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl border border-slate-900 bg-slate-900/40 hover:bg-slate-900 text-slate-200 hover:text-white transition-all text-xs font-bold active:scale-95 disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
              >
                {/* SVG for Google logo */}
            ...
            <div className="relative my-4 flex py-1 items-center">
              ...
              <span className="flex-shrink mx-3 text-[10px] uppercase font-semibold text-slate-600 tracking-wider">Atau Masuk dengan Email</span>
              <div className="flex-grow border-t border-slate-900"></div>
            </div>
